...
| [`check-release.mjs`](check-release.mjs) | Invoked by `npm run release:check -- <mode>`. | Checks draft, validator, or stable-release metadata and immutable artifacts without tagging or publishing. |
...
The commands do not create branches or pull requests, merge changes, create or move tags, publish GitHub releases, alter npm dist-tags, or publish packages. Run them on a release branch, review every generated change, run `npm test`, and use annotated tags only after explicit maintainer approval.
