...
| `packages/validator/src/snapshots/0.8.0-draft.4/**`, `packages/validator/test/snapshots/0.8.0-draft.4/**`, validator registry, declarations, tests, package metadata, and documentation | Added an immutable Draft 4 validator selector with embedded schema digest `93ed03f74059b5b3ce7509a96b59161bdab2c3cf7734397a9bec5a7588d0b03b`, selected cumulative validator `0.4.0` for npm `latest`, and preserved Draft 1–3 selectors | Make Draft 4 validation reproducible while keeping publication subject to the Draft 4 specification tag, tarball review, and explicit validator release tag |
...
## MCP Description v0.8.0 Release Candidate 1 — released 2026-08-31

Release Candidate 1 (`v0.8.0-rc.1`) selected Community Working Draft 4 as the baseline for final interoperability and release review. It added editorial clarification and release infrastructure without changing the `mcpdesc: 0.8.0` document shape, superseding stable v0.7.0, or changing `schemas/latest.json`.

| File or path | Change | Reason |
|---|---|---|
| `spec/draft/sections/**`, `spec/draft/mcp-description.md`, `spec/draft/guides/**`, `spec/draft/CHANGELOG.md` | Clarified reference mechanisms and schema identity, removed active Cisco-specific extension material, and marked the assembled specification as Release Candidate 1 | Incorporate final-review editorial feedback while keeping normative text and companion guidance synchronized |
| `schemas/mcp-description/0.8.0.json`, `schemas/draft.json`, `spec/draft/examples/**`, `spec/draft/fixtures/**` | Assigned the immutable `0.8.0-rc.1` schema identity and updated active document references | Make RC.1 documents and validation artifacts identify the exact prerelease schema resource |
| `scripts/prepare-release-candidate.mjs`, `scripts/prepare-release.mjs`, `scripts/check-release.mjs`, `scripts/schema-publication.mjs`, `scripts/test-schema-publication.mjs`, `scripts/README.md`, `package.json` | Added release-candidate preparation, metadata checks, and canonical publication verification | Make candidate preparation reproducible and prevent tagging before exact schema bytes and required response metadata are available |
| `packages/validator/**` | Added the immutable `0.8.0-rc.1` selector, standalone strict-CSP browser entry, provenance checks, and cumulative validator `0.5.0` release metadata | Provide reproducible executable validation for RC.1 while preserving earlier snapshot selectors |
| `README.md`, `spec/README.md`, `GOVERNANCE.md`, `specification-status.json` | Advanced public and machine-readable status from Draft 4 to Release Candidate 1 | Keep repository status aligned with `v0.8.0-rc.1` |

## MCP Description v0.8.0 Release Candidate 2 — prepared 2026-09-04

Release Candidate 2 (`v0.8.0-rc.2`) retains the Draft 4 proposal baseline and RC.1 document shape. It corrects official MCP extension recognition and defines reproducible extension-catalogue diagnostics. It remains a prerelease, does not supersede stable v0.7.0, and does not change `schemas/latest.json`.

| File or path | Change | Reason |
|---|---|---|
| `spec/draft/sections/08-capabilities.md`, `spec/draft/mcp-description.md`, `spec/draft/guides/**`, `spec/draft/CHANGELOG.md` | Defined official, experimental, and uncatalogued MCP-reserved extension handling; required catalogue source, effective-date, and maturity disclosure; and kept deeper extension validation in explicitly selected, version-pinned profiles | Remove false authority warnings without coupling core MCP Description conformance to independently evolving extension semantics |
| `scripts/validator-base.mjs`, `scripts/validate-0.8.mjs`, `scripts/test-views.mjs`, `spec/draft/fixtures/**` | Added a pinned official-extension catalogue recognizing Apps, Tasks, OAuth Client Credentials, and Enterprise-Managed Authorization; added distinct maturity diagnostics and Apps regression coverage | Recognize authoritative identifiers consistently in server capabilities and primitive client requirements while preserving warnings for uncatalogued reserved names |
| `schemas/mcp-description/0.8.0.json`, `schemas/draft.json`, `spec/draft/examples/**`, `spec/draft/fixtures/**` | Assigned the immutable `0.8.0-rc.2` schema identity and updated active document references without changing the schema's accepted document shape | Bind RC.2 artifacts to the exact candidate schema resource |
| `README.md`, `spec/README.md`, `GOVERNANCE.md`, `specification-status.json`, `spec/draft/PROPOSALS.md` | Advanced public and machine-readable status from RC.1 to prepared RC.2 while retaining the Draft 4 proposal provenance | Keep status and proposal-baseline disclosures aligned with the candidate |
